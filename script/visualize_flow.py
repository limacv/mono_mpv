"""
read the first frame and forward warp using flow
"""
...
for videofile in videos:
    ...
    for idx in range(framenum):
        ...
        if imglast is not None:
            ...
            print(f'\r{idx}', end='')
            if out.isOpened():
                imgwarp_vis = (imgwarp[0] * 255).type(torch.uint8).permute(1, 2, 0).cpu().numpy()
                imgwarp_vis = cv2.cvtColor(imgwarp_vis, cv2.COLOR_RGB2BGR)
                out.write(imgwarp_vis)
            else:
                if os.path.exists(videofile.replace('\\test\\', '\\visflow_warp\\')):
                    break
                else:
                    out.open(videofile.replace('\\test\\', '\\visflow_warp\\'),  828601953, 10., (wid, hei), True)
        else:
            imgwarp = img

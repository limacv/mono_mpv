...
from .LBTC_ConvLSTM import ConvLSTM


class TransformNet(nn.Module):

    def __init__(self, netcnl,
                 conv_cnl_num=32,
                 normlayer="IN",  # or "BN" "IN"
                 res_block_num=5
                 ):
        ...
        self.blocks = res_block_num
        self.epoch = 0
        nf = conv_cnl_num
        use_bias = (normlayer == "IN")

        # convolution layers
        self.conv1a = ConvLayer(netcnl * 2, nf * 1, kernel_size=7, stride=1, bias=use_bias,
                                norm=conv_cnl_num)  # input: P_t, O_t-1
        self.conv1b = ConvLayer(3 + 3, nf * 1, kernel_size=7, stride=1, bias=use_bias,
                                norm=conv_cnl_num)  # input: I_t, I_t-1
        self.conv2a = ConvLayer(nf * 1, nf * 2, kernel_size=3, stride=2, bias=use_bias, norm=conv_cnl_num)
        self.conv2b = ConvLayer(nf * 1, nf * 2, kernel_size=3, stride=2, bias=use_bias, norm=conv_cnl_num)
        self.conv3 = ConvLayer(nf * 4, nf * 4, kernel_size=3, stride=2, bias=use_bias, norm=conv_cnl_num)

        # Residual blocks
        self.ResBlocks = nn.ModuleList()
        for b in range(self.blocks):
            self.ResBlocks.append(ResidualBlock(nf * 4, bias=use_bias, norm=conv_cnl_num))

        # LSTM
        self.convlstm = ConvLSTM(input_size=nf * 4, hidden_size=nf * 4, kernel_size=3)

        # Upsampling Layers
        self.deconv1 = UpsampleConvLayer(nf * 4, nf * 2, kernel_size=3, stride=1, upsample=2, bias=use_bias,
                                         norm=conv_cnl_num)
    ...
    def forward(self, X, prev_state):
        ...
        Xa = X[:, :self.netcnl*2]  # P_t, O_t-1
        Xb = X[:, self.netcnl*2:]  # I_t, I_t-1
